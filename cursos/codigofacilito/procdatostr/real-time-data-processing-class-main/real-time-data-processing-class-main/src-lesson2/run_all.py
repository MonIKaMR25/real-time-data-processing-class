...
async def ensure_database_ready(timeout: float = 20.0, interval: float = 1.0) -> None:
    ...
    raise SystemExit(
        "CockroachDB is not ready on localhost:26257 or the 'orders' table is missing.\n"
        "Start Lesson 2 first with 'docker compose up -d' and wait for 'crdb-init' to finish.\n"
        f"Last check failed with: {detail}"
    )
